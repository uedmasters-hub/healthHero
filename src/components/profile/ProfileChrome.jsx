...
export function NavRow({ icon, label, onClick, href, progress }) {
  ...
  const progressPercent = Math.min(100, Math.max(0, Number(progress?.percent) || 0))
  return (
    <Tag
      type={href ? undefined : 'button'}
      className={`profile-nav-row${progress ? ' has-progress' : ''}`}
      onClick={onClick}
      href={href}
    >
      ...
      {progress ? (
        <span className={`profile-nav-progress profile-nav-progress--${progress.tone || 'partial'}`}>
          <span className="profile-nav-progress-meta">{progress.statusLabel}</span>
          <span className="profile-nav-progress-track" aria-hidden="true">
            <span className="profile-nav-progress-fill" style={{ width: `${progressPercent}%` }} />
          </span>
        </span>
      ) : null}
